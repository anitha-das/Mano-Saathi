...
import { cardClass, headingClass, mutedText, inputClass, primaryBtn, emptyStateClass } from "../styles/common";

function CounselorChats() {
  ...
  const getChats = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/counselor-api/chats`, { withCredentials: true });
  ...
  const sendMessage = async () => {
    try {
      ...
      const res = await axios.put(`${API_BASE_URL}/counselor-api/chats`, { chatId: activeChat._id, message: message.trim() }, { withCredentials: true });
      const updatedChat = { ...activeChat, ...res.data.payload, student: activeChat.student };
  ...
  return (
    <div className={cardClass}>
      <h2 className={headingClass}>Private Student Chat</h2>
      <div className="flex flex-wrap gap-2 mt-4">
        {chats.map((chatObj) => (
          ...
          >
            {chatObj.student?.firstName || "Student"}
          </button>
        ))}
      </div>

      {!activeChat && <p className={emptyStateClass}>Accepted student chats will appear here.</p>}
      {activeChat && (
        <div className="mt-5 overflow-hidden rounded-lg border border-[#DDE7D8] bg-[#F7F8F1] shadow-inner">
          <div className="border-b border-[#DDE7D8] bg-[#FFFDF7] px-5 py-4">
            <p className="font-bold text-[#123F31]">{activeChat.student?.firstName || "Student"} {activeChat.student?.lastName || ""}</p>
          ...
          <div className="min-h-80 max-h-[32rem] overflow-y-auto scroll-smooth p-4 sm:p-5 space-y-4">
                ...
                <div key={msg._id} className={`flex ${isOwnMessage ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] sm:max-w-[72%] rounded-2xl px-4 py-3 shadow-[0_8px_22px_rgba(24,76,56,0.08)] ${isOwnMessage ? "rounded-br-sm bg-[#0E4A37] text-white" : "rounded-bl-sm bg-[#FFFDF7] border border-[#DDE7D8] text-[#123F31]"}`}>
                    <p className={`text-xs font-bold mb-1 ${isOwnMessage ? "text-[#D9EADB]" : "text-[#1F7A58]"}`}>
                      {isOwnMessage ? "You" : activeChat.student?.firstName || "Student"}
                    </p>
            ...
            {activeChat.messages?.length === 0 && <p className={mutedText}>Begin with a supportive reply.</p>}
          </div>
