  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        { message: userText, history: messages.slice(-12) },
        { withCredentials: true },
      );
      setMessages([...nextMessages, { role: "model", text: res.data.payload.reply }]);
    } catch (err) {
      setMessages([...nextMessages, { role: "model", text: err.response?.data?.message || "AI Saathi could not respond right now. Please try again." }]);
    } finally {
      setLoading(false);
    }
  };
